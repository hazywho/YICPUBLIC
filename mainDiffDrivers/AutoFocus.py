import cv2
from positionMotorsLegacy import baseplateDrivers, focusStepper
import dotenv
...
class main():
    def __init__(self, slideArea: float = 3.24, waterData: dict = {"eee"}, camPort:int = 0):
        ...
        #setup focusMotor
        self.focusMotor = focusStepper(GPIO_pins=(12,16,20,21)) #4.7 rotations to go full range
        self.focusMotor.start()
        #setup baseplateDrivers
        self.baseplate = baseplateDrivers(delay=0.001, GPIO_pins=[(10,27,22,17),(26,6,13,5)])
    ...
    def getBestImg(self, minClearVariance = 80): #calibration+img Changed HEREEE!!!!!!!!! original:40
        try:
            counter=0
            lastBestVar=0
            self.bestVarPos=0
            ...
            #start loop
            while counter<self.focusMotor.step_per_revolution*2.5:
            ...
            self.focusMotor.goTo(0) #go back to position of best accuracy
    ...
    def getGemeniResponse(self, data: list, images: list, random_sample=False, selection=5): #get gemeni response
        if random_sample:
    ...
    def moveAroundAndProcess(self,x_res=5,y_res=5): #n+havent been used. Use it next update.
        listImg = []
        listAnnotatedImg = []
        ...
        #it takes 400 steps per revolution, 2.5 revolution seems to be the limit.
        # thus, 400*2.5, then divide by resolution of our map.
        # limitx = int(400*2.5)
        # limity = 5200
        ...
        print(dataJSON)
        response = self.getGemeniResponse(images=listImg, data=dataJSON, random_sample=False)
        print(response)
        return response, fullLstImg, listAnnotatedImg
