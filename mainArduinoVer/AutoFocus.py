import cv2
from positionMotorsArduino import baseplateDrivers, focusStepper
import dotenv
...
class main():
    def __init__(self, slideArea: float = 3.24, waterData: dict = {"eee"}, camPort:int = 0):
        ...
        self.baseplate = baseplateDrivers(delay=0.01, GPIO_pins=(10,11), port="/dev/ttyUSB1")
    ...
    def _createPlanktonData(self, image: list, slideArea): #function to create report output from image list. EDIT LATER: train our model first, then get our desired preds
        ...
        if len(imageContainsPlanktons) == 0:
            imageContainsPlanktons = image
            
        return plank_dat, imageContainsPlanktons #calculates plankton data only.
    
    def getGemeniResponse(self, data: list, images: list, random_sample=False, selection=1): #get gemeni response
    ...
    def moveAroundAndProcess(self,x_res=12,y_res=12): #n+havent been used. Use it next update.
        listImg = []
        listAnnotatedImg = []
        ...
        self.focusMotor.goTo(0)
        for x in range(0, limitx, resolution[0]): #move ins steps of limit.
            for y in range(0, limity, resolution[1]):
